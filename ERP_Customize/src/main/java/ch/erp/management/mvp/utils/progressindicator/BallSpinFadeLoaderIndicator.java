...
public class BallSpinFadeLoaderIndicator extends BaseIndicatorController {
    ...
    int[] alphas = new int[]{ALPHA, ALPHA, ALPHA, ALPHA, ALPHA, ALPHA, ALPHA,
            ALPHA};
    ...
    public void draw(Canvas canvas, Paint paint) {
        ...
        for (int i = 0; i < 8; i++) {
            canvas.save();
            Point point = circleAt(getWidth(), getHeight(), getWidth() / 2
                    - radius, i * (Math.PI / 4));
        ...
        for (int i = 0; i < 8; i++) {
            ...
            scaleAnim
                    .addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                        ...
                        public void onAnimationUpdate(ValueAnimator animation) {
                            scaleFloats[index] = (Float) animation
                                    .getAnimatedValue();
                            postInvalidate();
                        }
                    });
            ...
            alphaAnim
                    .addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                        @Override
                        public void onAnimationUpdate(ValueAnimator animation) {
                            alphas[index] = (Integer) animation
                                    .getAnimatedValue();
                            postInvalidate();
                        }
                    });
